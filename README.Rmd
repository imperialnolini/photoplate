...
The goal of the photoplate package is to customize stargazer::stargazer() regression table outputs (in R) for inclusion in LaTeX documents; you can specify tables' position arguments and internal labels for cross-referencing.
...
* *stargazer_call*: Call of stargazer::stargazer() on regression model output(s). DO NOT enter arguments for the keyword parameters "type" and "out" (though technically it's ok to have values for them which would yield a latex output, but it's probably easier to leave them out entirely). Also best not to identify this with the stargazer_call argument; just include the call itself.
...
In the future, I plan to update the package for cases when users might want to suppress the default stargazer::stargazer() stars for significance levels. For now, though, the package assumes your model output will have them.
