...
Prior to using photoplate(), it's a good idea to check that form of textual output - especially if you use the various stargazer::stargazer() parameters to format the output (e.g., renaming things and/or adding notes, as done here).
...
In general, the photoplate::photoplate() function seems to work best when "stargazer_call = "  is NOT included in its function call, so best to leave that out and simply enter the stargazer::stargazer() call as the first parameter without explicitly identifying it as such.
